...
`group` is an explicit, user-supplied identifier. Exposures sharing the same `group` across clusters are treated as **one logical exposure**. It is deliberately **not** derived from `spec.hostname` so that renaming a hostname does not silently break the failover relationship. Two failover-enabled Exposures in the **same namespace** MUST NOT share a `group` (controller surfaces `Reason=FailoverGroupConflict`, no CF write). The guard is per-namespace, not cluster-wide: across clusters the two group members live in separate apiservers and may legitimately share a namespace name, so a cluster-wide check cannot be expressed without false-positiving a valid pair.
...
- Two same-namespace failover Exposures sharing a `group` both go `Ready=False, Reason=FailoverGroupConflict` with no Cloudflare write.
