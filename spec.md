...
3. Ensure the desired Access application set exists when `access.enabled: true`:
   ...
   - Delete owned Access applications that are no longer desired. For non-failover Exposures, deletion is limited to status-tracked apps. For failover Primary cleanup, also discover group-owned apps under `spec.hostname` because a newly-promoted site may not have the former Primary's `status.cloudflare.accessApplications[]` entries. Never delete or mutate foreign-tagged apps.
...
4. Drift and deletion: update only owned apps whose domain/policy hashes differ; remove owned apps that are no longer desired; `access.enabled: false` and finalizer cleanup delete owned apps and preserve foreign-tagged apps. Non-failover cleanup is status-tracked; failover Primary cleanup also discovers live group-owned apps under `spec.hostname` because status is per-cluster while the app set is shared.
