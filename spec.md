...
| Standby (Manual) | expired / absent, no force token | leave alone | leave alone | reconcile (warm) | read only; `Reason=AwaitingPromotion` + Warning Event + pending metric |
...
- **Metrics** (Prometheus via controller-runtime registry):
  ...
  - `cfzt_failover_promotion_pending{exposure_group, site_id}` (gauge 0/1; `1` while a `promotionPolicy: Manual` Standby is declining to auto-promote and awaiting a deliberate force-promote; Slice 6, D26)
