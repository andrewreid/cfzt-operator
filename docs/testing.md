...
16. Create a DNS-only wildcard Exposure `*.wildcard-<run>.<zone>`, verify the proxied wildcard CNAME, and assert the operator wrote a wildcard tunnel ingress rule into the `CloudflareTunnel` status routes. The concrete subdomain `foo.wildcard-<run>.<zone>` is **not** probed over HTTPS — it is two labels deep and outside Universal SSL, so the edge has no certificate (see the edge-TLS note below); routing is validated via DNS + tunnel-ingress config instead.
17. Create a standalone wildcard self-hosted Access Exposure `*.access-wc-<run>.<zone>` (root + admin apps), verify the Access application IDs, domain hashes, and policy bindings, and verify the proxied wildcard CNAME. The covered concrete subdomain `foo.access-wc-<run>.<zone>` is **not** probed with an HTTPS Access challenge, for the same deep-wildcard edge-certificate reason.
...
Edge TLS for proxied wildcards: a proxied wildcard exposure is served over HTTPS only where an edge certificate covers the hostname. Cloudflare Universal SSL covers the apex and a single wildcard level (`*.example.com`, serving `foo.example.com`); a multi-label-deep wildcard (`*.sub.example.com`, serving `foo.sub.example.com`) has no Universal SSL certificate and its HTTPS handshake fails at the edge — a covering edge certificate requires Advanced Certificate Manager / Total TLS. The smoke's wildcard hosts are two labels deep, so the smoke validates deep-wildcard routing via DNS + tunnel-ingress config rather than an HTTPS reachability probe.
