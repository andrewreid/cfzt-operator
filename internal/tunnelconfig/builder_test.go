...
func TestTunnelConfigBuilderDeterministic(t *testing.T) {
	result, err := Build([]cfztv1alpha1.CloudflareExposure{
		exposure("b", "media", "z.example.com", "z.media.svc.cluster.local", "uid-z"),
		exposure("a", "media", "a.example.com", "a.media.svc.cluster.local", "uid-a"),
	})
	if err != nil {
...
func TestTunnelConfigBuilderHostnameCollision(t *testing.T) {
	_, err := Build([]cfztv1alpha1.CloudflareExposure{
		exposure("a", "media", "same.example.com", "a.media.svc.cluster.local", "uid-a"),
		exposure("b", "media", "same.example.com", "b.media.svc.cluster.local", "uid-b"),
	})
...
func TestTunnelConfigBuilderSkipsUnresolvedSourceRef(t *testing.T) {
	unresolved := exposure("route", "media", "", "route.media.svc.cluster.local", "uid-route")
...
func exposure(name, namespace, hostname, host, uid string) cfztv1alpha1.CloudflareExposure {
