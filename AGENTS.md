...
- All changes go through a feature branch and PR into `main`; required validation must pass before merge.

### Branching

- Branch names are human, work-descriptive names such as `fix/orphan-recovery`, `docs/operator-handbook`, or `feat/access-policy-recovery`.
- Do not use agent/tool prefixes or identifiers in branch names.

### Agent Attribution

- Never attribute work to the agent in commits, branches, PRs, docs, generated files, comments, release notes, or other repo artifacts.
- Do not use `Codex`, `Claude`, `AI`, agent names, co-author trailers, "generated by" text, or similar markers unless the user explicitly asks for that attribution.
